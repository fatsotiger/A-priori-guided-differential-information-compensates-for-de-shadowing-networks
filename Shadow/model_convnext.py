...
from modules.deablock_train import DEBlockTrain
...
class knowledge_adaptation_convnext(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            DEBlockTrain(default_conv, 1024, 3),
            DEBlockTrain(default_conv, 1024, 3),
        )
        self.conv2= nn.Sequential(
            DEBlockTrain(default_conv, 256, 3),
            DEBlockTrain(default_conv, 256, 3),
        )
        self.conv3 = nn.Sequential(
            DEBlockTrain(default_conv, 192, 3),
            DEBlockTrain(default_conv, 192, 3),
        )
        self.conv4 = nn.Sequential(
            DEBlockTrain(default_conv, 112, 3),
            DEBlockTrain(default_conv, 112, 3),
        )
        self.conv5 = DEBlockTrain(default_conv, 28, 3)
    ...
    def forward(self, input):
        b,c,h,w = input.shape # 1 3 750 1000
        if h % 256 !=0 or w % 256!=0:
            h_ = 256-(h % 256)
            w_=256-(w % 256)
            input = F.pad(input, (0, w_, 0, h_), mode='constant', value=0)
        # print(input.shape)
        x_layer1, x_layer2, x_output = self.encoder(input)
        # print(x_layer1.shape, x_layer2.shape, x_output.shape)

        x_mid = self.attention0(x_output)  # [1024,24,24]
        x_mid = self.conv1(x_mid)
...
class fusion_net(nn.Module):
    def __init__(self):
        ...
        self.fusion = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(28, 3, kernel_size=7, padding=0), nn.Tanh())
